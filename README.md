...
展开的用例里，**每个步骤一行、可独立展开收起**：收起时只看步骤 id、动作、状态、耗时与摘要（失败步骤默认展开），展开后看错误、完整输出、原始数据和该步骤的证据。页尾「证据总览」再按「用例 / 步骤」分组折叠一份同样的证据，便于整体翻查。

证据保存到 `artifacts/runs/<run_id>/evidence/`：浏览器步骤失败产生的 PNG 截图和文本快照，以及 ssh / mysql / http 等**非浏览器步骤的 stdout/stderr 文本**（默认采集；若要关闭，在该步骤上写 `"evidence": []`）。详情页内联预览 PNG 与文本类证据（txt/log/json/md），不需要先下载；非图片的预览统一按 `text/plain` 返回，避免证据被当成可执行文档。每份证据仍保留「下载原文件」链接，接口也支持 `?inline=1` 在浏览器里直接打开。
